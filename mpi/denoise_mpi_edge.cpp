...
int main(int argc, char **argv)
{
    ...
    {
        if (rank == 0)
            cerr << "Usage: mpirun -np <p> ./mpi_denoise_edge <image_path>\n";
        MPI_Finalize();
        return 1;
    }
    Mat fullImg;
    ...
    if (rank == 0)
    {
        imwrite("mpi_denoised.png", finalDenoised);
        imwrite("mpi_edges.png", finalEdges);
        cout << "MPI execution time: " << maxTime << " ms\n";
        cout << "Processes: " << size << "\n";
    }
    MPI_Finalize();
    return 0;
